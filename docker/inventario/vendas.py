...
graphs = {}
graphs['c'] = Counter('python_request_operations_total', 'Número total de requisição.')
...
ENDPOINT_CLIENTES = os.environ['ENDPOINT_CLIENTES']
ENDPOINT_CATALOGO = os.environ['ENDPOINT_CATALOGO']
...
def vendas_produ(id_cliente):
    try:
        ...
        cliente = requests.get(url = f'http://{ENDPOINT_CLIENTES}/clientes/{id_cliente}', headers = {'Authorization':'Basic ZG91Z2xhczoxMjM='})
        ...
        for i in (linha):
            if i not in produtos:
                ...
                catalogo = requests.get(url = f'http://{ENDPOINT_CATALOGO}/catalogo/{id_produto}', headers = {'Authorization':'Basic ZG91Z2xhczoxMjM='})
...
if __name__ == "__main__":
    app3.run(debug=True, host='0.0.0.0', port=5400)